...
def eval_split(cnn_model, model, crit, loader, eval_kwargs={}):
    ...
    caption_model = eval_kwargs.get('caption_model', 'show_tell')
    ...
    while True:
        ...
        if data.get('labels', None) is not None:
            ...
            loss = crit(model_output, labels[:,1:], masks[:,1:]).data[0]
        ...
        for k, sent in enumerate(sents):
            ...
            # you can generate attention visualization figure when caption models is show_attend_tell or topdown
         #   '''
            if split!='val':
                # Plot original image
                img = imread(os.path.join(eval_kwargs['image_root'], data['infos'][k]['file_path']))
                ...
                for t in range(len(words)):
                    ...
                    plt.axis('off')
                #plt.show()
                plt.savefig('atten_%s_%d.jpg' %(split,k))
                plt.clf()
            ##=======================================================
          #  '''
